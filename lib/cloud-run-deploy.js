 ...
 */
async function checkCloudRunServiceExists(
  projectId,
  location,
  serviceId,
  progressCallback
...
async function deployToCloudRun(
  ...
  try {
    ...
    if (exists) {
      ...
      await logAndProgress(`Creating new service ${serviceId}...`, progressCallback);
      [operation] = await callWithRetry(
        () =>
          runClient.createService({
            parent: parent,
 ...
 */
async function uploadToStorageBucket(
  bucket,
  buffer,
  destinationBlobName,
  progressCallback
 ...
 */
async function ensureArtifactRegistryRepoExists(
  projectId,
  location,
  repositoryId,
  format = 'DOCKER',
